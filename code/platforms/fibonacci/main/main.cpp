...
static void setup_wasm() {
    ...
    auto *wasm = (uint8_t *) wasm_test_c_wasm;
    uint32_t fsize = wasm_test_c_wasm_len - 1;

    env = m3_NewEnvironment();
    if (!env) FATAL("m3_NewEnvironment failed");

    runtime = m3_NewRuntime(env, 4 * 1024, NULL);
    ...
    result = m3_FindFunction(&f, runtime, "testfunction");
